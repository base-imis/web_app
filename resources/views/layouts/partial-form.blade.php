                    ...
                    <div class="form-group row @if($field->required) required @endif" @if(!empty($field->hidden)) @if($field->hidden === true) style="display: none" @endif @endif>
                        ...
                        <div class="col-sm-4">
                            @if($field->inputType === 'text')
                                {!! Form::text($field->inputId,$field->inputValue,['class' => $field->inputClass, 'placeholder' => $field->placeholder,'disabled' => $field->disabled,'autocomplete'=>$field->autoComplete]) !!}
                                @if($field->disabled)
                                {!! Form::hidden($field->inputId,$field->inputValue) !!}
                                @endif
                            @endif
                            @if($field->inputType === 'number')
                                {!! Form::number($field->inputId,$field->inputValue,['class' => $field->inputClass, 'placeholder' => $field->placeholder,'disabled' => $field->disabled]) !!}
                            @endif
                            ...
                            @if($field->inputType === 'date')
                                {!! Form::date($field->inputId,$field->selectedValue,['class' => $field->inputClass,'disabled' => $field->disabled,'autocomplete'=>'off']) !!}
                            @endif
        ...
        <div class="form-group row @if($formField->required) required @endif" @if(!empty($formField->hidden)) @if($formField->hidden === true) style="display: none" @endif @endif>
            ...
            <div class="col-sm-4">
                @if($formField->inputType === 'text')
                    {!! Form::text($formField->inputId,$formField->inputValue,['class' => $formField->inputClass, 'placeholder' => $formField->placeholder,'disabled' => $formField->disabled,'autocomplete'=>$formField->autoComplete]) !!}
                @endif
                ...
                @if($formField->inputType === 'number')
                    {!! Form::number($formField->inputId,$formField->inputValue,['class' => $formField->inputClass, 'placeholder' => $formField->placeholder,'disabled' => $formField->disabled]) !!}
                @endif
                ...
                @if($formField->inputType === 'date')
                    {!! Form::date($formField->inputId,$formField->inputValue,['class' => $formField->inputClass]) !!}
                @endif
                    @if($formField->inputType === 'year')
                        ...
                        @push('scripts')
                            <script>
                                $(document).ready(function () {
                                    console.log($('#{{ $formField->inputId }}').parent());
                ...
                @if($formField->inputType === 'file_viewer')
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <a href="{{ $formField->fileUrl }}" target="_blank"><button type="button" class="btn btn-info"><i class="fa-solid fa-eye"></i></button></a>
                        </div>
                        <input type="text" class="form-control" disabled value="{{ $formField->labelValue }}">
                    </div>
                @endif
