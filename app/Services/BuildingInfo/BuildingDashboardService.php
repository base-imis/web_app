...
class  BuildingDashboardService
{
    ...
    public function getContainmentTypesByBldgUseResidentials()
    {
		...
		ROUND(a.count * 100/b.totalward) as percentage_proportion
                FROM ( select ct.type, count(c.*), b.ward
                ...
                join fsm.containment_types ct ON c.type_id = ct.id
                where b.functional_use_id = 1 AND b.deleted_at IS NULL group by ct.type, b.ward
                     ) a
    ...
    //     $containment_types = DB::select("SELECT ct.type
    //         FROM fsm.containments as c
    //          Left join fsm.containment_types ct ON c.type_id = ct.id
    //         WHERE ct.type != 'No Containment'
    //         AND deleted_at IS NULL
    //         GROUP BY ct.type");
    //     $types = array();
    ...
    //             FROM ( select b.structure_type_id, ct.type, count(c.*)
    //              from building_info.buildings b join building_info.build_contains bc on b.bin = bc.bin
    //             join fsm.containments c on bc.containment_id = c.id
    //             join fsm.containment_types ct ON c.type_id = ct.id
    //             where structure_type_id is not null group by ct.type, b.structure_type_id
    //                  ) a
