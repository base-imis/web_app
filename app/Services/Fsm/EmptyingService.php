    ...
    public function getEditFormFields($emptying)
    {
        ...
        $this->editFormFields = [
            ...
            new FormField(
                ...
                labelFor: 'emptied_date',
                inputType: 'label',
                inputId: date('Y-m-d', strtotime($emptying->emptied_date)),
                placeholder: 'Date',
            ),
            new FormField(
                label: 'Service Receiver Name',
    ...
    public function createEmptying(Request $request)
    {
        ...
        try {
            if ($request->validated()) {
                ...
                $containments = Containment::leftJoin('building_info.build_contains', 'building_info.build_contains.containment_id', '=', 'fsm.containments.id')
                    ->where('building_info.build_contains.bin', $application->bin)
                    ->get(['fsm.containments.*']); 
                ...
                // Loop through each containment and update
        ...
        try {
            if ($request->validated()) {
                ...
                if($request->distance_closest_well){
                    ...
                    $containments = Containment::leftJoin('building_info.build_contains', 'building_info.build_contains.containment_id', '=', 'fsm.containments.id')
                    ->where('building_info.build_contains.bin', $application->bin)
                    ->get(['fsm.containments.*']); // Get all columns from the containment table
    ...
    public function export(Request $request)
    {
        ...
        $query->chunk(5000, function ($emptyings) use ($writer) {
            foreach($emptyings as $emptying) {
                ...
                $values[] = $emptying->distance_closest_well??"-";
