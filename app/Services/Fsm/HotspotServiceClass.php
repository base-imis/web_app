...
class HotspotServiceClass
{
        ...
        return Datatables::of($HotHotspot)
            ...
            ->addColumn('action', function ($model) {
                ...
                if (Auth::user()->can('Edit Hotspot Identification')) {
                    $content .= '<a title="Edit" href="' . action("Fsm\HotspotController@edit", [$model->id]) . '"  class="btn btn-info btn-sm mb-1"><i class="fa fa-edit"></i></a> ';
                }
                if (Auth::user()->can('View Hotspot Identification')) {
                    $content .= '<a title="History" href="' . action("Fsm\HotspotController@history", [$model->id]) . '" class="btn btn-info btn-sm mb-1"><i class="fa fa-history"></i></a> ';
                    $content .= '<a title="Detail" href="' . action("Fsm\HotspotController@show", [$model->id]) . '"class="btn btn-info btn-sm mb-1"><i class="fa fa-list"></i></a> ';
                }
    ...
    public function storeData($request)
    {
        ...
        if (!empty($request->geom)) {
            ...
            if (!empty($contains) && $contains === true) {
                ...
                return redirect('fsm/hotspots/create')->with('error', 'The selected area should be within the City Boundary');
            }
        } else {
        ...
        if ($Hotspots) {
            ...
            if (!empty($request->geom)) {
                ...
                if (!empty($contains) && $contains === true) {
                    ...
                    return redirect('fsm/hotspots/' . $id . '/edit')->with('error', 'The selected area should be within the City Boundary');
                }
            }
    ...
    */
    public function exportData($data)
    {
        ...
        $columns = ['Infected Disease', 'Hotspot Location', 'Date', 'No. of Cases','Male Cases','Female Cases','Other Cases','No of Fatalities','Male Fatalities','Female Fatalities','Other Fatalities'];
        $query = Hotspots::select('disease', 'hotspot_location', 'date', 'no_of_cases','male_cases','female_cases','other_cases', 'no_of_fatalities','male_fatalities','female_fatalities','other_fatalities')->whereNull('deleted_at');
