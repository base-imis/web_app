...
class MapsService {
   ...
   /**
    ...
    public function getBuildingToContainment() 
    {
        ...
        $query = "SELECT ST_Y (ST_Transform (geom, 4326)) as lat, ST_X (ST_Transform (geom, 4326)) as long"
            ...
            . " ON bc.bin = b.bin::CHARACTER VARYING"
            . " WHERE b.bin = ?"
            . " )";
    ...
    public function getContainmentToBuildings()
    {
        ...
        $query = "SELECT bin, ST_AsText(geom) AS geom"
            ...
            . " ON bc.containment_id = c.id::CHARACTER VARYING"
            . " WHERE c.id = ?"
            . " )";
    ...
    public function getContainmentBuildings($field, $value) 
    {
        ...
        $query = "SELECT bin, ST_Y (ST_Transform (ST_Centroid(geom), 4326)) as lat, ST_X (ST_Transform (ST_Centroid(geom), 4326)) as long"
            ...
            . " FROM building_info.build_contains bc"
            . " JOIN fsm.containments c"
            . " ON bc.id = c.id::CHARACTER VARYING"
            . " WHERE c." . $field . " = '" . $value . "'"
            . " )";
     ...
     *               assessment status, emptying status, feedback status, sludge collection status,
    ...
    public function getApplicationContainments()
    {
        ...
        if (Auth::user()->hasRole('Service Provider - Admin') || Auth::user()->hasRole('Service Provider - Help Desk') ) {
            ...
            $whereUser = " WHERE a.service_provider_id = " . Auth::user()->service_provider_id;
        } else {
        ...
        $query = "SELECT s.company_name, a.id AS application_id,a.bin,a.application_date, a.emptying_status, a.feedback_status, a.sludge_collection_status, c.id AS containment_id, ST_X(c.geom) AS long, ST_Y(c.geom) AS lat"
    ...
    {
        // Initialize the $where variable with a condition that is always true
        $where = " WHERE 1=1";
        // Add conditions based on the $year variable
        if ($year) {
            $where .= " AND extract(year from application_date) = '$year'";
        }
        // Add conditions based on the $month variable
        if ($month) {
            $where .= " AND extract(month from application_date) = '$month'";
        }
        ...
        $query = "SELECT a.id AS application_id,a.bin, s.company_name, a.emptying_status, a.feedback_status, a.sludge_collection_status, c.id AS containment_id, ST_X(c.geom) AS long, ST_Y(c.geom) AS lat"
            . " FROM fsm.applications a"
            . " LEFT JOIN building_info.buildings b"
            . " ON a.bin = b.bin"
            . " LEFT JOIN building_info.build_contains bc"
    ...
    public function getApplicationNotTPOnDate($start_date)
    {
        ...
        $query = "SELECT a.id AS application_id,a.application_date,a.bin,e.emptied_date,s.id,s.company_name, a.emptying_status, a.feedback_status, c.id AS containment_id, ST_X(c.geom) AS long, ST_Y(c.geom) AS lat"
            . " FROM fsm.applications a"
            . " LEFT JOIN building_info.buildings b"
            . " ON a.bin = b.bin"
            . " LEFT JOIN building_info.build_contains bc"
        ...
        $service_providers = DB::select("SELECT s.company_name"
            . " FROM fsm.applications a"
            . " LEFT JOIN building_info.buildings b"
            . " ON a.bin = b.bin"
            . " LEFT JOIN building_info.build_contains bc"
    ...
    public function getApplicationOnDate($start_date)
    {
        ...
        $query = "SELECT a.id AS application_id,a.bin, s.company_name, a.emptying_status, a.feedback_status, a.sludge_collection_status, c.id AS containment_id, ST_X(c.geom) AS long, ST_Y(c.geom) AS lat"
            ...
            . " LEFT JOIN fsm.service_providers s"
            . " ON s.id = a.service_provider_id"
            . " WHERE application_date = '$start_date' $whereUser";
    ...
    public function getApplicationNotTP()
    {
        ...
        if (Auth::user()->hasRole('Service Provider - Admin') || Auth::user()->hasRole('Service Provider - Help Desk') ) {
         ...
         // Construct the SQL query to retrieve application data along with related information:
        $query = "SELECT a.id AS application_id,a.bin,a.application_date,s.id,s.company_name,e.emptied_date, c.id AS containment_id, ST_X(c.geom) AS long, ST_Y(c.geom) AS lat"
            . " FROM fsm.applications a"
            . " LEFT JOIN building_info.buildings b"
            . " ON a.bin = b.bin"
            . " LEFT JOIN building_info.build_contains bc"
            ...
            . " LEFT JOIN fsm.emptyings e"
            . " ON e.application_id = a.id"
            . " WHERE a.emptying_status=true $whereUser"
            . " AND a.sludge_collection_status = false AND a.deleted_at is null";
        ...
        $service_providers = DB::select("SELECT s.company_name"
            ...
            . " ON e.application_id = a.id"
            . " WHERE a.emptying_status=true $whereUser"
            . " AND a.sludge_collection_status = false"
    ...
    public function getApplicationNotTPContainmentsYearMonth($year, $month)
    {
        ...
        $where = "  WHERE a.emptying_status = true AND a.sludge_collection_status = false";
        ...
        if ($month) {
         ...
         // Construct the SQL query to retrieve application data along with related information:
        $query = "SELECT a.id AS application_id,a.bin,a.application_date,e.emptied_date,s.id,s.company_name, a.emptying_status, a.feedback_status, c.id AS containment_id, ST_X(c.geom) AS long, ST_Y(c.geom) AS lat"
            . " FROM fsm.applications a"
            . " LEFT JOIN building_info.buildings b"
            . " ON a.bin = b.bin"
            . " LEFT JOIN building_info.build_contains bc"
    ...
    public function getUniqueContainmentEmptiedCount($geom, $whereUser)
    {
          // Construct the SQL query to retrieve the count of unique containment units that have been emptied
           ...
           $query = "SELECT COUNT(DISTINCT(c.id)) AS total_count"
                ...
                . " LEFT JOIN building_info.buildings b"
                . " ON bc.bin = b.bin"
                . " LEFT JOIN fsm.applications ap"
                . " ON ap.bin = b.bin"
                . " LEFT JOIN fsm.feedbacks fb ON fb.application_id = ap.id"
                . " LEFT JOIN auth.users u"
                . " ON u.id = fb.user_id"
                . " LEFT JOIN fsm.service_providers s"
                . " ON s.id = ap.service_provider_id"
                . " WHERE (ST_Intersects(c.geom, ST_GeomFromText('" . $geom . "', 4326)))"
                . " $whereUser"
    ...
    public function getFeedbackSludgeCollectionTransportationService($geom, $whereUser)
    {
        $query = "SELECT ss.scale_value, ss.scale_name, COUNT(fb.id)"
                . " FROM fsm.satisfaction_scales ss "
                . " LEFT JOIN fsm.feedbacks fb ON ss.scale_value = fb.sldgclln_trptn_srvc_qlty"
                . " LEFT JOIN fsm.applications ap ON fb.application_id = ap.id"
                . " LEFT JOIN building_info.buildings b"
                . " ON ap.bin = b.bin"
                . " LEFT JOIN building_info.build_contains bc"
    ...
    public function getFeedbackAccelerationEfficiencyServiceDelevery($geom, $whereUser)
    {
        $query = "SELECT ss.scale_value, ss.scale_name, COUNT(fb.id)"
                . " FROM fsm.satisfaction_scales ss "
                . " LEFT JOIN fsm.feedbacks fb ON ss.scale_value = fb.bhvr_srvc_prvdr"
                . " LEFT JOIN fsm.applications ap ON fb.application_id = ap.id"
                . " LEFT JOIN building_info.buildings b"
                . " ON ap.bin = b.bin"
                . " LEFT JOIN building_info.build_contains bc"
    ...
    public function getFeedbackImpactCreatingPublicAwareness($geom, $whereUser){

        $query = "SELECT ss.scale_value, ss.scale_name, COUNT(fb.id)"
                . " FROM fsm.satisfaction_scales ss "
                . " LEFT JOIN fsm.feedbacks fb ON ss.scale_value = fb.imct_crtng_pblc_awrns"
                . " LEFT JOIN fsm.applications ap ON fb.application_id = ap.id"
                . " LEFT JOIN building_info.buildings b"
                . " ON ap.bin = b.bin"
                . " LEFT JOIN building_info.build_contains bc"
    ...
    {
        if (Auth::user()->hasRole('Service Provider - Admin') || Auth::user()->hasRole('Service Provider - Help Desk') ) {
            $whereUser = "a1.service_provider_id = " . Auth::user()->service_provider_id;
        } else {
            $whereUser = "1=1";
        }
        $query = "SELECT c.id, ST_X(c.geom) AS long, ST_Y(c.geom) AS lat"
            . " FROM fsm.containments c"
            . " JOIN building_info.build_contains bc"
            . " ON bc.containment_id = c.id"
            . " JOIN building_info.buildings b"
            . " ON b.bin = bc.bin"
            . " JOIN fsm.applications a1"
            . " ON a1.bin = b.bin"
            . " WHERE a1.deleted_at is null AND a1.emptying_status is false AND $whereUser AND a1.proposed_emptying_date BETWEEN ? AND ?";
